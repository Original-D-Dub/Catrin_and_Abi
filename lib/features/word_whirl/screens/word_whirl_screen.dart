...
import '../providers/word_whirl_provider.dart';

class WordWhirlScreen extends StatelessWidget {
  const WordWhirlScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<WordWhirlProvider>(
      builder: (context, provider, _) {
        ...
        return Scaffold(
          ...
          appBar: provider.showLevelSelect
              ? GameAppBar(
                  title: localizer('word_whirl.title'),
          ...
          body: Container(
            ...
            child: SafeArea(
              child: provider.showLevelSelect
                  ? LevelSelectScreen(
                      locale: locale,
                      subtitle: localizer('word_whirl.instructions'),
                      levels: WordWhirlLevel.forLocale(locale).map((level) {
                  ...
                  : Stack(
                      children: [
                        ...
                        if (provider.state == WordWhirlState.correct)
                          Positioned.fill(
                        ...
                        if (provider.state == WordWhirlState.complete)
                          Positioned.fill(
                        ...
                        if (provider.state == WordWhirlState.playing)
                          Positioned(
                            right: 16,
                            bottom: 16,
                            child: _ClearButton(
                              label: localizer('word_whirl.clear'),
            ...
            : Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      widget.localizer('word_whirl.video_coming_soon'),
                      style: const TextStyle(
...
class _ProgressDots extends StatelessWidget {
  final WordWhirlProvider provider;
      ...
      children: List.generate(provider.wordCount, (i) {
        final answered = i < provider.wordIndex ||
            (i == provider.wordIndex &&
                provider.state == WordWhirlState.correct);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return GestureDetector(
        onPanStart: (d) {
          if (provider.state != WordWhirlState.playing) return;
        ...
        child: CustomPaint(
          painter: _LinePainter(
            ...
            dragPos: provider.state == WordWhirlState.playing ? _dragPos : null,
